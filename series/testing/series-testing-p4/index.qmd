...
*You can also locate the path to Chromium using by holding command <kbd>⌘</kbd> and clicking on the application icon*
...
│       └── histogramApp-001_.png
├── setup-shinytest2.R
├── test-shinytest2-datasetApp.R <- new test file!
└── test-shinytest2.R
...
After saving, loading and building the package, I'll create the `test-shinytest2-selectVarApp.R` test file and add the initial contents to create the `AppDriver` object:
...
Note that in this test, I can load the development version of `selectVarApp()` with `system.file()` (this is a similar folder structure to apps built with [`golem`](https://thinkr-open.github.io/golem/articles/a_start.html) and [`leprechaun`](https://leprechaun.opifex.org/#/guide/build) frameworks) 
...
I'll use `app$get_values()` to store the exported `input`/`output`/`export` test values in `app_values`:
...
I'll replicate the example from the website using the `gghistApp()`:
...
-   The plot is verified with the exported `hist-plot_obj` and `expect_doppelganger()` from the `vdiffr` package. 
    ...
    ── Warning (_test-shinytest2-gghistApp.R:45:3): {shinytest2}: gghistApp ───
...
-   This initial run of this test will show a warning again as the snapshot file is saved to `tests/testthat/_snaps/`:
    ...
    ── Warning (_test-shinytest2-gghistApp.R:77:3): {shinytest2}: gghistApp ───
