...
I've organized the app-package development process into three areas: **Start**, **Build**, and **Use**. 
  ...
  -   **Run** shows how to launch a `golem` application locally (i.e., within the RStudio IDE), common workflow tips, and anything I found confusing while building the application.
...
#| echo: false
#| results: asis
co_box(color = "g", "**Tip**: development scripts", contents = "
...
In `data-raw/movies.R`, I'll load and add the movies data to the `data/` folder
...
Follow [this guide](https://cran.r-project.org/web/packages/roxygen2/vignettes/rd-other.html) to document your datasets. The `ggplot2` package has [a great example.](https://github.com/tidyverse/ggplot2/blob/main/R/data.R)
...
There are three test files in `tests/testthat/` at this point: 
...
These files test the functions in the files `golem_utils_ui.R` and `golem_utils_server.R`. 
...
#| results: asis
co_box(color = "b",
header = "`golem_utils` functions: **NOTE**", 
contents = "
...
This concludes the `02_dev.R` file. You'll return to this file repeatedly during development for various functions (`add_module()`, `add_utils()`, and `add_fct()`), so it's a good idea to leave this .R file in the `dev/` folder. 

## Use 

In this section, I'll go over the functions used during application development, and to launch the application (locally, in the IDE). The third script (`dev/03_deploy.R`), is used to deploy the application. 

### Writing code

New modules and utility functions can be created with [`golem::add_module()`](https://thinkr-open.github.io/golem/reference/add_module.html):
...
-   The outputs from the initial `devtools::check()` are below (I've split it up into sections)
    ...
    ── R CMD build ──────────────────────────────────────────────────────────
    ✔  checking for file ‘/Users/mjfrigaard/projects/pkgs/apps/gap/DESCRIPTION’ ...
    ─  preparing ‘gap’: (561ms)
    ...
    X  Comparing ‘spelling.Rout’ to ‘spelling.Rout.save’ ...
       ...
       < ui                  
       tests_and_coverage.Rmd:37,41,54,74,74,75,75,76,76,77,77,78,78,79,79,80,80,81,
...
[1] "/Users/mjfrigaard/projects/pkgs/apps/gap_0.0.0.9000.tar.gz"
```
...
#| results: asis
co_box(color = "o",
header = "`rsconnect::deployApp(appName = 'gap-movies')`: **Recommend**", 
contents = "
...
Building an application with `golem` is very similar to developing an R package. New code files are placed in `R/`, external resources are placed in `inst/`, etc. 
