...
We can use the app stored in [`bbsa`](https://github.com/mjfrigaard/bbsa) to conduct an experiment. Below are the initial files: 
...
### Initial [`.Rproj`]{style="font-size: 1.10em"} file
...
These are settings for our shiny app project. Lets see what happens if we create an `R/` folder (with a `utils.R` file) and a `DESCRIPTION` file.
...
The contents of the `DESCRIPTION` file are below (adapted from [R Packages](https://r-pkgs.org/description.html#the-description-file)): 
...
`bbsa` now has the following files and folders: 
...
Let's take a quick look at the IDE to confirm none of the **Build** tools are available: 
...
We can see three new lines have been added to `bbsa.Rproj`: 
