...
subtitle: "Building a '*`golem`-light*' shiny app-package"
author: "Martin Frigaard"
date: "2023-07-20"
categories: [shiny, leprechaun]
image: "image.png"
...
This post is another walk-through of a shiny application using the  [`leprechaun` framework](https://leprechaun.opifex.org/#/). `leprechaun` apps are lightweight [`golem` apps](https://golemverse.org/), "*this means applications are leaner, and smaller; hence the name 'leprechaun.'*."  I'll go through developing a `leprechaun` shiny app-package (and how it's different from `golem` and regular (`devtools`) app-packages).
...
`leprechaun` apps are built using the same methods as R packages (`devtools` and `usethis`), but are intended to be a 'leaner and smaller' version of `golem`. The GitHub repo with the lap shiny app-package is [here](https://github.com/mjfrigaard/lap).
...
```{r}
#| label: create_package-myLeprechaunApp
#| code-fold: false
#| eval: false
...
::: {style="font-size: 0.80em;"}
```{bash}
#| label: create_package-myLeprechaunApp-output
#| code-fold: true
...
co_box(color = "b",
  ...
  contents = "The output above shows that--unlike `golem` apps--`leprechaun` includes the functions in the `R/` folder as part of the `myLeprechaunApp` package.")
```
