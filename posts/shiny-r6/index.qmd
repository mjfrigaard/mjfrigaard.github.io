...
Shiny apps accumulate complexity in two places: the UI tends to get wide; the server tends to get deep. In a large app the server becomes a tangle of reactive values, observers, and helper functions all sharing mutable state through `reactiveValues` objects passed around as implicit globals. Packaging the app as an R package helps with tooling; R6 classes help with architecture. Together they let you draw a clean line between "things that change at runtime" and "things that handle reactivity" — and test each side independently.

In this post we walk through `movexplR6`, a Shiny app-package that lets users explore movie ratings from a SQLite database. R6 manages the database connection and filtering logic; Shiny modules handle the UI and reactive wiring. We'll cover:
...
The key insight is that `MovieData` sits outside the reactive graph entirely. It knows nothing about Shiny; it just connects to a database, holds the data, and filters it on demand. The server function is the adapter between R6 and reactivity.

## What Is R6?

R has several object-oriented systems. S3 and S4 use **copy-on-modify** semantics: when you pass an object to a function and modify it inside, you modify a copy and the original is unchanged. R6 uses **reference semantics**: all variables that point to the same R6 object share the same underlying data. Modifying the object through one variable is immediately visible through all other variables that reference it.
...
A quick mini-experiment makes the difference concrete. If you assign an R6 object to two variables, both variables refer to the same object:

```{r}
#| label: ref-semantics
DemoClass <- R6::R6Class("DemoClass",
  public = list(
    value = 0,
    set = function(v) self$value <- v
  )
)
...
a$set(99)
b$value         # 99 — mutation through a is visible via b
```

This is different from how a plain list behaves and it is the property that makes R6 useful for Shiny state.
...
Below is the definition of the `MovieData` object (expand to view):
