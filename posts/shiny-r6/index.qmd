...
As a Shiny app-package grows, I've found it's easy to end up with state scattered across reactives, observers, and module arguments. Shiny's reactive model is naturally functional, but `R6` classes let us bundle state, behavior, and business logic into a single object. In an app-package, `{R6}` offers clean separation of concerns, enforced namespace isolation, and business logic we can test outside the app.

But `{R6}` in Shiny can take different forms. In this post, we'll compare two patterns:

1. **`{R6}` for modules** (`stocktipr6`): Wrap every module in an `R6` class with `$ui()` and `$server()` methods
2. **`{R6}` for Business Logic** (`movexplR6`): Keep modules function-based; use `{R6}` for stateful business logic

We'll explore what each pattern buys you, when to use each, and how they compare in practice.

## What is `{R6}`?

`{R6}` is an object-oriented system in R that creates reference-based objects. Unlike `S3` or `S4`, `R6` objects are mutable and support encapsulation (i.e., public and private members). If you've written classes in Python or Java, `R6` will feel familiar; methods belong to the object, and the object is modified in place (i.e., reference semantics).
...
Shiny itself is built on `{R6}` (reactive values and observers are `R6` classes under the hood), so using `R6` objects in an app-package extends a pattern Shiny already relies on.
...
1. **`stocktipr6`**: A stock volatility dashboard where each module is an `R6` class
2. **`movexplR6`**: A movie database explorer where modules stay function-based, but business logic lives in `{R6}`

Let's start with `stocktipr6` (`R6` modules) to see the architecture:

### `{R6}` for Modules 

Access the code here: [mjfrigaard.github.io/stocktipr6](https://mjfrigaard.github.io/stocktipr6/)

![](img/stocktipr6.png){width='100%' fig-align='center'}
...
4. **`R6` class bundling download UI and server logic; its `$ui()` card is placed in the `ModInputs` sidebar (equivalent to `mod_download.R` in `stocktipr`)**    
5. **`R6` class bundling inputs UI and server logic; returns reactive list from `$server()` method (equivalent to `mod_inputs.R` in `stocktipr`)**   
6. **`R6` class bundling outputs UI and server logic; consumes reactive inputs from `ModInputs` (equivalent to `mod_outputs.R` in `stocktipr`)**    
7. Helper that wraps hover/tooltip text for `reactable` table cells (identical to `stocktipr`)    
8. Tooltip icon helper component used in module labels (identical to `stocktipr`)     
...
Both patterns work, but they solve different problems. `stocktipr6` enforces architecture at the module level; `movexplR6` enforces it at the business logic layer. The choice depends on where your app's complexity lives.

### `R6` Modules 

In `stocktipr6`, each *module* is an `R6` class. When writing `R6` modules, both the UI and server logic are wrapped into a single **class**. Below is a simplified example from `stocktipr6` (view the full version [on GitHub.](https://github.com/mjfrigaard/stocktipr6/blob/main/R/ModInputs.R)).
...
Before going further, it's worth comparing how `R6` modules and function-based modules handle the namespace `id`.

#### `R6` Modules vs. standard functions

`stocktipr6` bundles both UI and server into a single `R6` class. `ModInputs$new("inputs")` creates one instance where the `id` is stored in `private$id` and automatically available to both the `ui()` and `server()` methods.
...
The `id` is **bound at instantiation and encapsulated** in the object. Both `$ui()` and `$server()` methods access it from the same source, so within an instance, the UI and server namespaces can't drift out of sync.
...
`movexplR6` shows a different pattern; it uses `{R6}` for resources and business logic and keeps the Shiny modules function-based. This cleanly separates concerns: stateful data access in `{R6}`, stateless UI/reactivity in functions.
...
## Comparison: stocktipr6 vs movexplR6

Both apps need to manage state, but they organize it differently.

### Public and Private Members

In `{R6}`, `public` methods are callable from outside the object; `private` members are internal. With full `R6` modules (`stocktipr6`):
...
The `id` and `ns` are shared across methods, so they're always in sync.
...
%%| eval: true
%%| fig-align: 'center'
%%| fig-caption: 'R6 Module Lifecycle (stocktipr6)'
%%| echo: false
%%{init: {'theme': 'base', 'themeVariables': { 'fontFamily': 'monospace', "darkMode":true}}}}%%
...
The `private` list holds internal state (`id` and `ns`). These are accessed by public methods, but they aren't part of the public interface (tests can still reach them through `.__enclos_env__`, as shown below).
...
### Full `R6` Modules (`stocktipr6`)

In `stocktipr6`, each module is an instance with its own private state. When `ModOutputs` needs data from `ModInputs`, it receives the reactive list returned by `ModInputs$server()` and consumes it as a parameter. 

```{mermaid}
%%| eval: true
%%| fig-align: 'center'
%%| fig-caption: 'State Flow: R6 Modules (stocktipr6)'
%%| echo: false
%%{init: {'theme': 'base', 'themeVariables': { 'fontFamily': 'monospace', "darkMode":true}}}}%%
...
Dependencies flow in one direction: downstream modules receive what they need from upstream instances, and can't modify the upstream module's private state. This one-way flow means a downstream module can't accidentally overwrite an upstream module's inputs.
...
When something breaks, the first thing I want to know is what state a module is holding. Each pattern answers that question differently.

### `R6` Objects (`stocktipr6`)
...
This tells us the data layer's state (the `con` connection and the `all_movies` data frame), methods, and lifecycle. The modules themselves remain lightweight.

## Why Encapsulation Matters

The `R6` approach (whether full modules or business logic layer) makes state ownership explicit. With full `R6` modules (`stocktipr6`):

- The module's identity and dependencies are clear: `inputs$server()` uses the same ID as `inputs$ui()`
- The UI and server namespaces can't drift apart; the ID is bound at instantiation
...
Without encapsulation (pure function-based), a typo like passing `"input"` instead of `"inputs"` to `mod_inputs_server()` creates a silent namespace mismatch. The app still launches, but the inputs never reach the server.

## Business Logic vs UI Encapsulation

The key difference between the two patterns:

- **stocktipr6 (full `R6` modules)**: Encapsulates UI/server logic. Each module is a self-contained unit.
...
## Using `R6` Modules (stocktipr6)

In the app's UI, instantiate the `R6` objects and call their `$ui()` methods. The server creates its own instances with the same `id` values, so both sides share the same namespace:
...
In `stocktipr6`'s `ModInputs`, the `server()` method returns a reactive list:
...
As your app-package scales, teammates and users need to understand module and class interfaces. Documenting `R6` classes with **[`roxygen2`](https://roxygen2.r-lib.org/)** makes `?ModInputs` or `?MovieData` complete references. `roxygen2` has supported `R6` classes since version 7.0.0.
...
With `roxygen2` and `{R6}` together, teammates can run `?ModInputs` and immediately understand what the class does, the methods that are available, what parameters they accept, and what they return. This is much faster than reading through `R/ModInputs.R`.
...
For full `R6` module patterns (like `stocktipr6`), use `CamelCase` names for classes and place each in its own file:
...
In both cases, export `R6` classes with `@export` so package users can instantiate them. Never instantiate objects at package load time; always do it inside `app_ui()`, `app_server()`, or module server functions to ensure one instance per session.

### Tests 

Writing tests with `testthat` is especially valuable for `R6` modules. I've found it's worth testing that instantiation works, the methods return expected types, and that passing different `id` values produces properly isolated namespaces:
...
* **stocktipr6**: [mjfrigaard.github.io/stocktipr6](https://mjfrigaard.github.io/stocktipr6/)    
