...
I've included the `R/` folder tree below to help understand how this application is built.
...
├── app_ui.R #<3>
├── compute_rolling_vol.R #<4>
├── custom_head.R #<5>
├── default_tickers.R #<6>
├── get_ff3_factors.R #<7>
├── get_stock_prices.R #<8>
├── get_stock_returns.R #<9>
├── launch.R #<10>
├── mod_download.R #<11>
├── mod_hoverinfo.R #<12>
├── mod_inputs.R #<13>
...
├── mod_tooltip.R #<15>
├── setup_theme.R #<16>
├── summarise_performance.R #<17>
├── utils_operators.R #<18>
├── utils_reactable_theme.R #<19>
└── with_logging.R #<20>
...
4. Computes rolling volatility metric for stock analysis.
5. Helper function for displaying data in a compact format.
6. Exports default ticker symbols as a package constant.
7. Fetches Fama-French 3-factor data from external source.
8. Fetches historical stock price data from API.
9. Calculates stock returns from price data.
10. App entry point; calls `shinyApp(ui, server)`.
11. Function-based download module; pairs `mod_download_ui()` and `mod_download_server(id)` in one file.
12. Helper that wraps hover/tooltip text for `reactable` table cells.
13. Function-based inputs module; contains `mod_inputs_ui(id)` and `mod_inputs_server(id)` returning reactive list.
14. Function-based outputs module; consumes `inputs_r` and renders performance metrics.
15. Tooltip icon helper component.
16. Theme and styling configuration.
17. Calculates performance summary metrics (returns, Sharpe ratio, etc.).
18. Custom operators (`%>%`, `%||%`, etc.).
19. `reactable` table theme utilities.
20. Logging wrapper utility for side-effect tracking.
