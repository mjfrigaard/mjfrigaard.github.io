...
I recently purchased a [System76](https://system76.com/) laptop, which comes with [Pop!\_OS](https://system76.com/pop/), an Ubuntu-flavored operating system. This post covers installing R, Python, and Quarto and setting up RStudio and Positron. 

## The COSMIC Desktop Environment 

[Pop!\_OS](https://github.com/pop-os/cosmic-epoch) features the COSMIC  (it previous used GNOME), a fully custom Rust-based desktop environment. As mentioned previously, COSMIC is built on Ubuntu, meaning it uses the same package ecosystem (i.e., `.deb` files and `apt` package manager). 

![](img/cosmic-de.png){width='100%' fig-align='center'}

:::{.column-margin}

Read more about the OS on the [System76 website](https://system76.com/pop/)
...
Installing Quarto (and getting it configured) required quite a few steps. Consult the [documentation](https://quarto.org/docs/get-started/) for more information. 
...
We will address the [TinyTex installation](https://quarto.org/docs/output-formats/pdf-engine.html#installing-tex) first with `quarto`:
...
Quarto also has a command for installing [Chromium](https://www.chromium.org/getting-involved/download-chromium/):
...
I installed the Java Runtime Environment (`default-jre`) to run Java applications like VeraPDF. On [Ubuntu/Pop!\_OS 24.04](https://system76.com/pop/), this will install OpenJDK 21, which exceeds VeraPDF's minimum requirement of Java 8.
...
![](img/rstudio-install.png){width='80%' fig-align='center'}
...
I've installed the following Positron extensions from the Positron extensions marketplace.[^positron-ext] None of these are required, but I've found them useful during R package/Shiny app development.[^positon-ext-pak]
...
1. [Shiny - VS Code Extension](https://open-vsx.org/extension/posit/shiny): Enables running, previewing, and debugging R and Python Shiny applications directly within Positron/VS Code.

2. [Posit Publisher](https://open-vsx.org/extension/posit/publisher): Deploys and publishes content (Shiny apps, Quarto documents, APIs, etc.) directly to Posit Connect from within the editor.

3. [Quarto - VS Code](https://open-vsx.org/extension/quarto/quarto): Provides syntax highlighting, preview, and rendering support for Quarto (`.qmd`) documents.

4. [Air - R Language Support](https://open-vsx.org/extension/posit/air-vscode): Provides R language support including real-time code formatting based on Posit's Air R formatter.

5. [Git Graph](https://open-vsx.org/extension/mhutchie/git-graph): Visualizes your Git repository's branch history and commit tree in an interactive graphical interface.


6. [GitHub Pull Requests](https://open-vsx.org/extension/github/vscode-pull-request-github): Allows you to create, review, and manage GitHub pull requests and issues directly within the editor.

7. [GitLens extension](https://open-vsx.org/extension/eamodio/gitlens): Enhances Git integration with inline blame annotations, commit history exploration, and repository insights.

8. [VSCode .env syntax highlighting ](https://open-vsx.org/extension/mikestead/dotenv): Adds syntax highlighting for `.env` environment variable files.

9. [Mermaid](https://open-vsx.org/extension/MermaidChart/vscode-mermaid-chart): Adds syntax highlighting and live preview support for Mermaid diagrams within markdown and `.mmd` files.

[^positon-ext-pak]: Most of these extensions come from the [Positron +1e extension pack](https://open-vsx.org/extension/grrrck/positron-plus-1-e) from [Garrick Aden-Buie](https://github.com/gadenbuie).

## Publishing

To confirm everything is working correctly, I published this post using Positron. In my next post, I'll dig a little deeper into my workflow in the COSMIC Desktop Environment (vs. macOS and Windows).
