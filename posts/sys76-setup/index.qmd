...
Below is an adaptation of the installation instructions from [Posit](https://docs.posit.co/resources/install-python-uv.html) (using `uv`).

Start by installing the official `uv` installer scrip. 
...
Use `uv` to install Python 3.12.4, which manages its own Python installations independently of the system Python. 
...
Start by downloading the latest Quarto `.deb` package from the [Quarto releases page](https://github.com/quarto-dev/quarto-cli/releases) (in this case it's **quarto-1.9.30-linux-amd64.deb**). 
...
Download the  `rstudio-2026.04.0-daily-368-amd64.deb` and double-click on it to install RStudio. 
...
Consider installing the [GitLens extension](https://open-vsx.org/extension/eamodio/gitlens) from the Positron extensions marketplace for enhanced Git capabilities — this provides inline blame annotations, commit history, and repository insights directly in the editor.
